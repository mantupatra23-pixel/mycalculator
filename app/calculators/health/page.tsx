...
export const metadata: Metadata = {
  title: "Health & Fitness Calculators - BMI, BMR & Calories | MyCalculators",
  description: "Free health calculators. Calculate BMI, BMR, daily maintenance calories, macros, ideal body weight and water intake goals.",
};

export default function HealthCategoryPage() {
  const calcs = CALCULATORS.filter((c) => c.category === "health");
  return (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center gap-2 text-xs font-semibold text-navy/60 mb-4">
        ...
        <span>/</span>
        <span className="text-navy">Health</span>
      </div>
      ...
      <div className="bg-sage/40 rounded-3xl p-6 sm:p-10 border border-navy/10 mb-10">
        ...
        <h1 className="text-3xl sm:text-4xl font-black text-navy mb-3">Health & Fitness Calculators</h1>
        <p className="text-navy/80 text-sm sm:text-base max-w-2xl leading-relaxed">
          Estimate Body Mass Index (BMI), Basal Metabolic Rate (BMR), daily calorie requirements, macros and ideal hydration targets.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        {calcs.map((item) => (
          <div key={item.id} className="bg-white border border-navy/15 rounded-2xl p-5 hover:border-steel hover:shadow-md transition-all flex flex-col justify-between">
            <div>
              <span className="text-[11px] font-bold uppercase tracking-wider text-steel bg-sage/50 px-2 py-0.5 rounded">Health Metric</span>
